...
// const filamat_shadow_url = 'shadow_test.filamat';
...
class App {
  constructor(canvas) {
    ...
    const matinstance2 = material.createInstance();
    matinstance2.setColor3Parameter('baseColor', Filament.RgbType.sRGB, [ 0.5, 0.5, 0.5 ]);
    matinstance2.setFloatParameter('roughness', 1.0);
    matinstance2.setFloatParameter('clearCoat', 0.0);
    matinstance2.setFloatParameter('clearCoatRoughness', 1.0);
    ...
    // const materialShadow = engine.createMaterial(filamat_shadow_url);
    // const matinstShadow = materialShadow.createInstance();
    addPlane(matinstance2);//, -0.1);
    // addPlane(matinstShadow);
    function addPlane(matinst, yoff = 0) {
      const vb = Filament.VertexBuffer.Builder()
        .vertexCount(4)
        .bufferCount(2)
        .attribute(VertexAttribute.POSITION, 0, AttributeType.FLOAT3, 0, 0)
      ...
      const r = 4;
      vb.setBufferAt(engine, 0, new Float32Array([
        -r, -1 + yoff, -r,
        -r, -1 + yoff,  r,
         r, -1 + yoff,  r,
         r, -1 + yoff, -r,
...

      {
        ...
        vb.setBufferAt(engine, 1, orientation.getQuats(4));
      ...
      const planeEntity = Filament.EntityManager.get().create();
  ...
  resize() {
    ...
    this.camera.setProjectionFov(45, width / height, 1.0, 100.0, Fov.VERTICAL);
  }
}
